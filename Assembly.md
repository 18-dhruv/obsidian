...
each computer that has CLR have machine wide code cashe called GAC. It stores assemblies specification designated to shared by several application on computer
...
### GAC is legacy now the problem with GAC was suppose I have two  apps they depend on same assembly we want to update dependencies for one app but because of that the second app breaks because it defends on that to solve três problem we use NuGet 
